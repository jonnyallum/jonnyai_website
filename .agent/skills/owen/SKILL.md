...
### Before Every Task
```
1. Query Shared Brain: Are all Quality Gates (Design, Truth, SEO, Security) green?
2. Check chatroom: Are there infrastructure alerts from @Derek?
3. Verify build: Is the local environment clean?
```

### After Every Task
```
1. Record outcome: Update deployment logs in Shared Brain.
2. Document friction: Note any build failures or server lag.
3. Capture learning: Propagate to @Owen and @Sebastian.
```
