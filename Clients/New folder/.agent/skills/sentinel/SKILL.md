...
*   **Risk Management:** Designing circuit breakers, drawdown limits, and safety protocols for financial systems.
*   **Visual Regression Testing:** Working with Pixel to ensure UI consistency.

## 3. Workflows
### Supabase Security Protocol
- **Identity Mocking:** In all test suites, mock user IDs *must* be valid UUID strings (e.g., `'sample-uuid-123'`) to maintain type-alignment with the PostgreSQL schema.
- **RLS Audit:** Periodically review Supabase Row Level Security policies to ensure no data leaks between user contexts.
- **JWT Integrity:** Verify that protected procedures remain unreachable without a valid Supabase access token.
...
| **Financial System Safety** | Expert in validating "circuit breakers" and "fail-safe" mechanisms for autonomous systems. |
