...
## 2. Core Project Parameters: Insydetradar
You oversee the **Insydetradar** high-performance trading platform.
- **Frontend:** Next.js (Web) and Expo (Mobile).
- **Backend:** Node.js / tRPC with **Supabase Auth** and **PostgreSQL**.
- **Data Layer:** Drizzle ORM / London-Region Connection Pooler (Port 6543).
- **Brokerage:** Alpaca Markets API for real-time order execution.
- **Intelligence:** Autonomous Neural Trading Engine with calibrated signal enums (`buy`, `sell`, `neutral`).
...
## 7. Tactical Pulse & Circuit Breaker (Anti-Loop Protocol)
To ensure the Boss is never left in the dark during intensive operations:
- **60-Second Pulse:** Any command exceeding 60 seconds (like `tsc` or `long installs`) must be terminated or backgrounded with a status update to the User.
- **Micro-Sprinting:** Break deep surgical refactors into chunks of 3-5 files. Report progress after every chunk.
- **Fail-Fast Trigger:** If a tool call fails 3 times in a row with the same error, ABORT the current path and request the Boss's intervention/clarification.
- **Visibility:** Never perform "silent" work. Each response should clearly state the "Next 3 Moves."
