...
def check_systems_safe():
    ...
    try:
        ...
        if r.status_code in [302, 303]:
            ...
            if 'accounts.google.com' in loc:
                print("[OK] Auth Config: SUCCESS (Redirects to Google)")
            else:
                print(f"[WARN] Auth Config: REDIRECTS to {loc}")
        elif r.status_code == 200:
             print("[INFO] Auth Config: Returned 200")
        else:
             print(f"[FAIL] Auth Config: FAILED ({r.text})")
    except Exception as e:
        print(f"[FAIL] Auth Config: Error {e}")
    ...
    payload = {"email": "system_check_auth_complete_v2@insydetradar.com"}
    
    print(f"\nChecking API URL: {api_url}")
    try:
        r = requests.post(api_url, headers=headers, json=payload)
        if r.status_code in [200, 201]:
            print("[OK] API Cache: SUCCESS (Lead inserted)")
        elif r.status_code == 404:
            print("[FAIL] API Cache: FAILED (404 - Table not found in cache)")
        else:
             print(f"[FAIL] API Cache: FAILED ({r.status_code} - 429/400 etc)")
    except Exception as e:
        print(f"[FAIL] API Cache: Error {e}")
